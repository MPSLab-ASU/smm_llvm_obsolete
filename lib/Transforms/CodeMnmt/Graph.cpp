...
Node::Node(std::string name, double p) :probability(p), name(name) {
    next = NULL;
}

Node * Node::add_next(std::string name, double p) {
    next = new Node(name, p);
    return next;
}


Edge * Node::add_edge_to(Node *dest, unsigned long num) {
    Edge *new_edge = new Edge(this, dest, num);
    back_edges.push_back(new_edge);
    return new_edge;
}


Edge::Edge(Node *src, Node *dest, unsigned long num): src(src), dest(dest), num(num) {
}
...
void dfs_visit(CallGraphNode::CallRecord *v, std::vector<CallGraphNode::CallRecord *>& exec_trace) {
	...
	if ( called_cgn->getFunction() && called_cgn != caller_cgn) {
